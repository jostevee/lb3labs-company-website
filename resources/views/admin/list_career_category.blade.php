      ...
      <div class="container-fluid">
        ...
        <div class="card-header">Category List</div>
          <div class="card-body">
              ...
              <a href="/admin/job/category/add" class="mb-3 btn btn-outline-info ml-2">Add Category</a><br><br>
              <br>
              <p>Total Category =
                {{ $category->count() }}
              </p>
              <table class="table table-striped">
                ...
                <tr>
                  <td>{{ $id++ }}</td>
                  <td>{{ $data->category }}</td>
                  <td>{{ $data->career()->count() }}</td>
                  <td>{{ $data->career()->groupBy('id')->count() }}</td>
                  <td>{{ $data->created_at }}</td>
                  <td>{{ $data->updated_at }}</td>
                  <td>
                    <a class="btn btn-warning btn-sm col-12 mb-2" href="/admin/job/category/edit/{{ $data->id }}">Edit</a>
                    <form action="/admin/job/category/delete/{{ $data->id }}" method="POST">@csrf
